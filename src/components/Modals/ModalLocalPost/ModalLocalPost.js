    ...
    return (
        <ModalWrapperChildPost title="Tìm kiếm vị trí">
            <div className="w-full my-2 px-2">
                ...
                pl-4 bg-transparent dark:bg-dark-third rounded-3xl" type="text"
                    placeholder="Tìm kiếm" search={true} />
            </div>
            <div className="tac-user wrapper-content-right">
                {locals.map(local =>
                    <div onClick={() => {
                        postsDispatch(postsAction.updateData('local', posts.local ? null : local));
                        postsDispatch(postsAction.returnModalPost());
                    }} key={local.id}
                        className={`w-full pl-4 rounded-lg cursor-pointer relative flex py-1.5
                        ${posts.local ? posts.local.id === local.id ? 'bg-gray-200 dark:bg-dark-third'
                                : 'hover:bg-gray-200 dark:hover:bg-dark-third' : 'hover:bg-gray-200 dark:hover:bg-dark-third'} `}>
