...
const ContentStory = () => {
    ...
    const { stories: { timeCurrent, current, isPlaying, indexRun, show, main, indexStory, storyList }, storiesDispatch, storiesAction } = useContext(StoryContext);
    ...
    return (
        <div className="w-7/12 story-right bg-gray-400 dark:bg-dark-third mt-5 relative m-2 rounded-lg relative"
            ...
            <audio src={JSON.parse(main.music).src} ref={refAudio} className='hidden' autoPlay />
            ...
            {current ? current.groupStory ? !show && user.id === current.groupStory.userGroupStory.id && < div onClick={() => storiesDispatch(storiesAction.updateData('show', true))} className='absolute -bottom-12 cursor-pointer left-2 p-2 z-50'>
            ...
            {show && user.id === current.groupStory.userGroupStory.id && <InfoStory setShow={(status) => storiesDispatch(storiesAction.updateData('show', status))} />}
        </div >
    )
}
